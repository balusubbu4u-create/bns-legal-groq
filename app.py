...
selected_model = st.sidebar.selectbox(
    "మోడల్‌ను ఎంచుకోండి:",
    options=["llama-3.1-8b-instant", "openai/gpt-oss-20b", "openai/gpt-oss-120b"],
    index=0,
    help="llama-3.1-8b-instant వేగంగా మరియు తప్పుల్లేకుండా JSON అవుట్‌పుట్ ఇస్తుంది."
)
...
def clean_and_parse_json(text):
    # 1. Remove reasoning / thought tags if generated by gpt-oss models
    ...
    # 3. Find outermost curly braces containing the JSON object
    start_idx = text.find('{')
    end_idx = text.rfind('}')
...
Task: Analyze the user complaint and uploaded document contents and extract strict statutory sections, procedural guidelines, evidence rules, and IO action checklists.
...
1. Strict Ingredient Matching:
   ...
   - BNS 319(2) (Cheating by personation): Max punishment is up to 5 years, or fine, or both. Cognizable, Bailable.
...
2. BNSS Procedure Guidelines:
   ...
   - Section 35(3) BNSS: Notice of Appearance is statutory before arrest for offences punishable with less than 7 years.
   - Section 187(3) BNSS: Statutory 60-day or 90-day threshold for default bail purposes.
   - Section 193 BNSS: Final report without unnecessary delay, and progress update every 90 days under Sec 193(3)(ii).

3. BSA Evidence Compliance:
   - Section 63(4) BSA: Require prescribed certification for electronic evidence.
   - Section 105 BNSS: Mandatory audio-video recording during search and seizure.
   - Section 176(3) BNSS: Mandatory forensic crime scene visit for offences with 7+ years punishment.

CRITICAL INSTRUCTION:
You must output ONLY valid JSON without preamble, thoughts, markdown ticks, or conversational text. Follow this schema:
{
    ...
    if not api_key:
        ...
        with st.spinner(f"ఫిర్యాదు వివరాలను `{selected_model}` ద్వారా విశ్లేషిస్తోంది..."):
            try:
                ...
                report_data = clean_and_parse_json(raw_output)
